...
async function main() {
	...
	const [voter, addr1] = await ethers.getSigners()
	console.log("Voting with account:", addr1.address)
	...
	const hasVoted = await governor.hasVoted(proposalId, addr1.address)
	if (hasVoted) {
	...
	const votingWay = 1
	const voteTx = await governor.connect(addr1).castVote(proposalId, votingWay)
	await voteTx.wait()
