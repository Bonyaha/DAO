...
const ProposalTimingButton = ({ proposal, governorAddress }) => {
	...
	useEffect(() => {
					...
					case 0: // Pending
						if (proposalSnapshot) {
							...
							if (blocksUntilActive > 0) {
								...
								setTooltipText(`Voting begins in ${blocksUntilActive} blocks`)
								setButtonStyle({
									bg: 'bg-amber-100',
									text: 'text-amber-800',
					...
					case 1: // Active
						if (proposalDeadline) {
							...
							if (blocksUntilDeadline > 0) {
								...
								setTooltipText(`Voting ends in ${blocksUntilDeadline} blocks`)
								setButtonStyle({
									bg: 'bg-blue-100',
									text: 'text-blue-800',
		...
		return () => clearInterval(interval)
	}, [proposal, publicClient, currentTime, proposalSnapshot, proposalDeadline, governorAddress])
...
const ProposalTimingButton = ({ proposal, governorAddress }) => {
	...
	useEffect(() => {
		...
		const interval = setInterval(calculateTimeLeft, 5000)
		return () => clearInterval(interval)
	}, [proposal, publicClient, currentTime, proposalSnapshot, proposalDeadline, governorAddress, blocksToTime, formatTimeLeft])
